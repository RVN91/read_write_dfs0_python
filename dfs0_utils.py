...
Requires python 2.7 due to clr libs.
...
def write_dfs0(gauge_time, gauge_int, out_file, script_folder):
    ...
    Input:
        gauge_time: Numpy array or list with time stamps.
					Each time stamp needs to be matplotlib
					date number (see date2num). (np.float)
        gauge_int: Numpy array or list with data. Each value
				   needs to be a. (np.float)
        out_file: File name for the dfs0 file (string)
        script_folder: Folder containing this shit (string)
